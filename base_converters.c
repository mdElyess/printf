...
/**
 * convert_sbase - Converts a signed long to an inputted base and stores the
 *		result to a buffer contained in a struct.
		...
		unsigned char flags, int wid, int prec)
{
	...
	{
		for (; prec > 1; prec--, wid--) /* Handle Percision */
			ret += _memcpy(output, &pad, 1);
		if (NEG_FLAG == 0) /* Handle Widht */
		{
			pad = (ZERO_FLAG == 1) ? '0' : ' ';
			for (; wid > 1; wid--)
				ret += _memcpy(output, &pad, 1);
 ...
 * @output: A buffer_t struct containing a character array.
 * @num: An unsigned long to a string  containing the base to convert to.
 * @base: A pointer to a string containing the base to convert to.
 * @flags: Flag modifiers.
 * @wid: A width modifier.
 * @prec: A precision modifier.
